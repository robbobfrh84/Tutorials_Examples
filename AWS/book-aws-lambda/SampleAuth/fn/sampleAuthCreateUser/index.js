...
function sendVerificationEmail(email, username, token, fn) {
		...
		Message: {
			...
			Body: {
				Html: {
					...
					+ '<br><br>'
					+ 'Hello '+username+',please <a href="' + verificationLink + '">' + verificationLink + '</a>'
					+ '</body></html>'
				}
			}
		}
		...
		if (err) {
			...
			storeUser(email, username, hash, salt, function(err, token) {
				if (err) {
					...
					sendVerificationEmail(email, username, token, function(err, data) {
						if (err) {
